...
def venues():
  ...
  #
  data = []
  state_and_cities =  Venue.query.with_entities(Venue.city, Venue.state).all()
  for res in state_and_cities:
    
    venues = Venue.query.filter_by(city=res.city, state=res.state).all()
    venues_data = []
...
def search_venues():
  ...
  data = []
  search_results = list(Venue.query.filter(
...
def search_artists():
    search_term = request.form.get('search_term', '')
    data = []
    search_results = list(Artist.query.filter(
